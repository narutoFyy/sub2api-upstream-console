...
async function api(path, options = {}) {
  ...
  if (!res.ok) {
    const details = data.details
      ...
      : '';
    throw new Error([data.error || `Request failed: ${res.status}`, details].filter(Boolean).join('锛?));
  }
  return data;
}
...
function renderImportResults(results = []) {
  document.querySelector('#importResults').innerHTML = results.length ? results.map((item) => `
    <div class="list-item">
      <strong>${escapeHtml(item.action === 'updated' ? '宸叉洿鏂? : '宸叉柊寤?)} 路 ${escapeHtml(item.name)}</strong>
      <small>${escapeHtml(item.base_url)} 路 ID ${escapeHtml(item.id)}</small>
    </div>
  `).join('') : '<p class="empty">娌℃湁瀵煎叆浠讳綍涓婃父銆?/p>';
}

async function importConfigFile(file) {
  if (!file) return;
  if (!file.name.toLowerCase().endsWith('.json') && file.type !== 'application/json') {
    throw new Error('璇烽€夋嫨 JSON 閰嶇疆鏂囦欢銆?);
  }
  const text = await file.text();
  if (!text.trim()) throw new Error('瀵煎叆鏂囦欢涓虹┖銆?);
  let payload;
  try {
    payload = JSON.parse(text);
  } catch (err) {
    throw new Error('瀵煎叆鏂囦欢涓嶆槸鏈夋晥鐨?JSON銆?);
  }
  if (!Array.isArray(payload?.sites)) {
    throw new Error('瀵煎叆鏂囦欢鏍煎紡涓嶆纭細闇€瑕佸寘鍚?sites 鏁扮粍銆?);
  }
  ...
  document.querySelector('#importSummary').textContent = `宸插鍏?${result.imported} 涓笂娓革細鎸?Base URL 鑷姩鏇存柊鎴栨柊寤恒€俙;
...
function setForm(site = null, credentials = {}) {
  ...
  document.querySelector('#formTitle').textContent = site ? `缂栬緫涓婃父锛?{site.name}` : '鏂板涓婃父';
  document.querySelector('#formMessage').textContent = site
    ? '姝ｅ湪缂栬緫宸叉湁涓婃父銆傚瘑鐮佹垨 Token 鐣欑┖鏃朵細淇濈暀鍘熷嚟璇併€?
    : '绔欑偣鏍囩鍙槸缁欎綘鑷繁鍒嗙被锛涘€嶇巼璇嗗埆璇嶇敤浜庤嚜鍔ㄥ垽鏂摢浜涘垎缁勫睘浜?Codex銆?;
}
...
function renderCards(totals) {
  const cards = [
    ['涓婃父鎬绘暟', totals.upstreams],
    ['姝ｅ父涓婃父', totals.active],
    ['鍚屾澶辫触', totals.failed],
    ['浠婃棩 Token', tokenText(totals.today_tokens)],
    ['浠婃棩鎴愭湰', money.format(totals.today_cost)],
    ['浣欓涓嶈冻', totals.low_balance],
    ['鍊嶇巼鎻愰啋', totals.unacknowledged_changes ?? state.dashboard?.changes?.length ?? 0],
    ['鍚庡彴鍚屾', '宸插紑鍚?]
  ];
...
function renderRechargeAlerts(alerts = []) {
  ...
  container.innerHTML = alerts.map((item) => {
    const methods = (item.payment_methods || []).map((method) => paymentMethodLabel(method.type)).join('銆?);
    return `
      <div class="list-item">
        <strong>${escapeHtml(item.name)} 路 浣欓 ${money.format(item.balance)} / 闃堝€?${money.format(item.threshold)}</strong>
        <small>寤鸿鑷冲皯鍏呭€?RMB ${money.format(item.suggested_amount)} 路 ${item.payment_supported ? `瀹樻柟鍙敤锛?{escapeHtml(methods)}` : '璇ヤ笂娓镐笉鏀寔鍦ㄧ嚎鍏呭€?}</small>
        <button class="ghost" data-detail-id="${item.id}">鏌ョ湅璇︽儏</button>
      </div>
    `;
  }).join('');
}
...
function siteWarnings(site) {
  ...
  if (Number.isFinite(balance) && balance > 0 && balance < threshold) warnings.push(`浣欓浣庝簬 ${threshold}`);
  if (site.last_sync_error) warnings.push(site.last_sync_error);
  if (!site.last_sync_at) warnings.push('灏氭湭鍚屾');
  if (isStaleSite(site)) warnings.push('闀挎湡鏈悓姝?);
  return warnings;
}
    ...
    return `
      <tr>
        <td>
          ...
          <small>${escapeHtml((site.tags || []).join(' 路 '))}</small>
        </td>
        ...
        <td class="${isLowBalanceSite(site) ? 'danger-text' : ''}">
          ${site.balance ?? '涓嶅彲鐢?} <small>${escapeHtml(site.balance_currency || '')}</small>
        </td>
        <td>
        ...
        <td>
          ...
          ${warnings.length ? `<small class="danger-text">${escapeHtml(warnings.join('锛?))}</small>` : ''}
        </td>
        <td class="actions">
          <button class="ghost" data-detail-id="${site.id}">璇︽儏</button>
          <button class="ghost" data-copy-url="${escapeHtml(site.base_url)}">澶嶅埗</button>
          <button class="ghost" data-sync-id="${site.id}">鍚屾</button>
          <button class="ghost" data-edit-id="${site.id}">缂栬緫</button>
          <button class="ghost" data-status-id="${site.id}" data-next-status="${site.status === 'disabled' ? 'active' : 'disabled'}">${site.status === 'disabled' ? '鍚敤' : '鍋滅敤'}</button>
          <button class="ghost danger" data-delete-id="${site.id}">鍒犻櫎</button>
        </td>
      </tr>
    `;
  }).join('');
  document.querySelector('#upstreamRows').innerHTML = rows || '<tr><td colspan="11" class="empty">杩樻病鏈夊尮閰嶇殑涓婃父銆?/td></tr>';
}

function renderRateChanges(changes) {
  document.querySelector('#rateChanges').innerHTML = changes.length ? changes.map((item) => {
    const percent = item.change_percent === null || item.change_percent === undefined ? '' : ` 路 ${money.format(item.change_percent)}%`;
    return `
        <div class="list-item">
          <strong>${escapeHtml(item.upstream_name)} 路 ${escapeHtml(item.group_name || item.group_id)}</strong>
        <small>${rateText(item.old_rate)} -> ${rateText(item.new_rate)}${percent} 路 ${timeText(item.detected_at)}${item.acknowledged_at ? ` 路 宸茬‘璁?${timeText(item.acknowledged_at)}` : ''}</small>
        ${item.acknowledged_at ? '' : `<button class="ghost" data-ack-rate-change="${item.id}">纭</button>`}
      </div>
    `;
  }).join('') : '<p class="empty">鏆傛椂娌℃湁鍊嶇巼鍙樺寲銆傞娆″悓姝ヤ細寤虹珛鍩虹嚎銆?/p>';
}

function renderLogs(logs) {
  document.querySelector('#syncLogs').innerHTML = logs.length ? logs.map((item) => `
    <div class="list-item">
      <strong>${escapeHtml(item.upstream_name)} 路 ${escapeHtml(item.status)}</strong>
      <small>${escapeHtml(item.summary || item.error_message || '鏃犳憳瑕?)} 路 ${item.duration_ms}ms 路 ${timeText(item.started_at)}</small>
    </div>
  `).join('') : '<p class="empty">鏆傛棤鍚屾鏃ュ織銆?/p>';
}
...
function renderRates() {
  ...
  document.querySelector('#rateGrid').innerHTML = latest.length ? latest.slice(0, 48).map((rate) => `
    <article class="rate-pill">
      ...
      <strong>${escapeHtml(rate.group_name || rate.group_id || '鏈懡鍚嶅垎缁?)}</strong>
      <span>${rateText(rate.rate)}</span>
      <small>${rate.model ? `${escapeHtml(rate.model)} 路 ` : ''}${escapeHtml(rate.scope || '鏈爣娉ㄥ钩鍙?)}</small>
    </article>
  `).join('') : '<p class="empty">娌℃湁鍖归厤鐨勫€嶇巼銆傚彲浠ユ崲涓瓫閫夋潯浠讹紝鎴栧厛鍚屾涓婃父銆?/p>';
}

function renderUpstreamPriceRow(item) {
  const isRequest = Number(item.quota_type || 0) === 1;
  const priceText = isRequest
    ? usdText(item.upstream_request_usd, '/ 娆?)
    : `杈撳叆 ${usdText(item.upstream_input_usd_per_1m)} 路 杈撳嚭 ${usdText(item.upstream_output_usd_per_1m)}`;
  const cacheText = !isRequest && Number.isFinite(Number(item.upstream_cache_read_usd_per_1m))
    ? `缂撳瓨璇?${usdText(item.upstream_cache_read_usd_per_1m)}`
    : '';
  return `
    <div class="upstream-price-row">
      <strong>${escapeHtml(item.upstream_name || `涓婃父 #${item.upstream_site_id}`)}</strong>
      <span>${escapeHtml(priceText)}</span>
      <small>鍒嗙粍 ${escapeHtml(item.effective_group || 'default')} 路 鍒嗙粍鍊嶇巼 ${rateText(item.effective_group_ratio || 1)}${cacheText ? ` 路 ${escapeHtml(cacheText)}` : ''}</small>
    </div>
  `;
}
...
function renderModelPriceCard(model) {
  ...
  const officialText = isRequest
    ? usdText(model.official_request_usd, '/ 娆?)
    : `杈撳叆 ${usdText(model.official_input_usd_per_1m)} 路 杈撳嚭 ${usdText(model.official_output_usd_per_1m)}`;
  return `
    <article class="model-board-card">
      <div class="model-board-head">
        <div>
          <strong>${escapeHtml(model.model_name)}</strong>
          <small>${escapeHtml(model.vendor || '')}${model.tags ? ` 路 ${escapeHtml(model.tags)}` : ''}</small>
        </div>
        <span><em>瀹樻柟鍘熶环</em>${escapeHtml(officialText)}</span>
      </div>
      <div class="upstream-price-list">
        ${(model.upstreams || []).map(renderUpstreamPriceRow).join('') || '<p class="empty">鏆傛棤涓婃父浠锋牸銆?/p>'}
      </div>
    </article>
  `;
}
...
function renderModelPricingBoard() {
  ...
  const sections = [
    ['OpenAI 妯″瀷', board.openai || []],
    ['Claude 妯″瀷', board.claude || []]
  ];
  document.querySelector('#modelPricingBoard').innerHTML = sections.map(([title, models]) => `
    <section class="model-board-section">
      <div class="model-board-title">
        ...
        <span>${models.length} 涓ā鍨?/span>
      </div>
      <div class="model-board-grid">
        ${models.length ? models.map(renderModelPriceCard).join('') : '<p class="empty">鏆傛棤鍙姣旂殑妯″瀷浠锋牸銆傝鍏堝悓姝ユ敮鎸?/api/pricing 鐨?new-api 涓婃父銆?/p>'}
      </div>
    </section>
  `).join('');
}
...
function trendSvg(history) {
  ...
  if (items.length < 2) return '<p class="empty">瓒嬪娍鏁版嵁杩樹笉澶燂紝鑷冲皯鍚屾涓ゆ鍚庝細鏄剧ず鎶樼嚎銆?/p>';
  const width = 720;
  const height = 180;
  const fields = [
    ['balance', '浣欓', '#0d6b5f'],
    ['today_tokens', '浠婃棩 Token', '#dc7b2f']
  ];
  ...
  return `<svg class="trend" viewBox="0 0 ${width} ${height}" role="img" aria-label="瓒嬪娍鍥?>${lines}</svg>`;
}
...
function renderCapabilities(capabilities = {}) {
  ...
  return items.map(([key, label]) => `
    <span class="capability ${capabilities[key] ? 'ok' : 'missing'}">${label}锛?{capabilities[key] ? '鏀寔' : '涓嶅彲鐢?}</span>
  `).join('');
}
...
function renderRechargePanel(detail) {
  ...
  if (!supported) {
    return `
      <h3>鍦ㄧ嚎鍏呭€?/h3>
      <p class="empty">璇ヤ笂娓镐笉鏀寔鍦ㄧ嚎鍏呭€硷紝鎴栧凡鍏抽棴浣欓鍏呭€笺€?/p>
    `;
  }
  ...
  return `
    ...
    <div class="recharge-box">
      <div class="recharge-form">
        <label>鍏呭€奸噾棰?RMB<input data-recharge-amount="${detail.site.id}" type="number" min="1" step="0.01" value="10" /></label>
        <label>鏀粯鏂瑰紡
        ...
        <button class="secondary" data-create-recharge="${detail.site.id}">鍒涘缓鍏呭€艰鍗?/button>
      </div>
      <p class="detail-note">鍙皟鐢ㄤ笂娓稿畼鏂硅鍗曟帴鍙ｃ€傛敮浠樼‘璁や粛鐢辨敮浠樺疂銆佸井淇℃垨涓婃父鏀堕摱鍙板畬鎴愩€?/p>
      <div id="rechargeOrderBox-${detail.site.id}">
        ${latestOrder ? renderRechargeOrder(latestOrder) : '<p class="empty">鏆傛棤鍏呭€艰鍗曘€?/p>'}
      </div>
    </div>
  `;
}
...
function renderRechargeOrder(order) {
  ...
  const qrImage = qr.startsWith('http') || qr.startsWith('data:image')
    ? `<img class="payment-qr" src="${escapeHtml(qr)}" alt="鏀粯浜岀淮鐮? />`
    : '';
  const qrText = qr && !qrImage ? `<code class="qr-text">${escapeHtml(qr)}</code>` : '';
  return `
    <div class="payment-order">
      <strong>璁㈠崟 #${escapeHtml(order.upstream_order_id || order.id)} 路 ${escapeHtml(order.status || 'PENDING')}</strong>
      <small>鏀粯 ${money.format(order.pay_amount || order.amount || 0)} 路 ${escapeHtml(order.payment_type || '')} 路 ${timeText(order.created_at)}</small>
      ${qrImage}
      ${qrText}
      <div class="form-actions">
        ${payUrl ? `<a class="button-link" href="${escapeHtml(payUrl)}" target="_blank" rel="noreferrer">鎵撳紑鏀堕摱鍙?/a>` : ''}
        <button class="ghost" data-refresh-recharge="${order.id}">鍒锋柊璁㈠崟鐘舵€?/button>
      </div>
    </div>
  `;
}
...
function modelPricingValue(item) {
  if (Number(item?.quota_type) === 1 && Number.isFinite(Number(item?.upstream_request_usd))) {
    return `${money.format(Number(item.upstream_request_usd))} / 娆;
  }
  if (Number.isFinite(Number(item?.upstream_input_usd_per_1m)) || Number.isFinite(Number(item?.upstream_output_usd_per_1m))) {
    ...
    return `杈撳叆 ${input} 路 杈撳嚭 ${output}`;
  }
  if (Number(item?.quota_type) === 1 && Number.isFinite(Number(item?.model_price))) {
    return `${money.format(Number(item.model_price))} 鍥哄畾浠锋牸`;
  }
...
function detailPricingMeta(item) {
  const parts = [item.vendor || '鏈煡渚涘簲鍟?];
  if (item.effective_group) parts.push(`鍒嗙粍 ${item.effective_group}`);
  if (Number.isFinite(Number(item.effective_group_ratio))) {
    parts.push(`鍊嶇巼 ${money.format(Number(item.effective_group_ratio))}x`);
  }
  return parts.join(' 路 ');
}
...
function renderSubscriptionPanel(snapshot) {
  ...
  if (!summary.enabled || !summary.primary) {
    return `
      <h3>璁㈤槄濂楅</h3>
      <p class="empty">鏆傛棤娲昏穬璁㈤槄濂楅銆?/p>
    `;
  }
  const sub = summary.primary;
  const planName = sub.plan_title || `濂楅 #${sub.plan_id || 'n/a'}`;
  ...
  return `
    ...
    <div class="detail-grid">
      <article class="metric"><span>娲昏穬 / 鍏ㄩ儴</span><strong>${activeCount} / ${totalCount}</strong></article>
      <article class="metric"><span>濂楅</span><strong>${escapeHtml(planName)}</strong></article>
      <article class="metric"><span>璁㈤槄 ID</span><strong>#${escapeHtml(sub.id || 'n/a')}</strong></article>
      <article class="metric"><span>鐘舵€?/span><strong>${escapeHtml(sub.status || '鏈煡')}</strong></article>
      <article class="metric"><span>鎵ｈ垂绛栫暐</span><strong>${escapeHtml(summary.billing_preference || '鏈煡')}</strong></article>
      <article class="metric"><span>鍓╀綑澶╂暟</span><strong>${sub.days_remaining ?? 'n/a'}</strong></article>
      <article class="metric"><span>鍒版湡鏃堕棿</span><strong>${escapeHtml(dateTimeFromSeconds(sub.end_time))}</strong></article>
      <article class="metric"><span>涓嬫閲嶇疆</span><strong>${escapeHtml(dateTimeFromSeconds(sub.next_reset_time))}</strong></article>
      <article class="metric"><span>鎬婚搴?/span><strong>${money.format(total)}</strong></article>
      <article class="metric"><span>宸茬敤棰濆害</span><strong>${money.format(used)}</strong></article>
      <article class="metric"><span>鍓╀綑棰濆害</span><strong>${money.format(remaining)}</strong></article>
      <article class="metric"><span>浣跨敤姣斾緥</span><strong>${money.format(percent)}%</strong></article>
    </div>
  `;
}
...
function renderDetail(detail) {
  const snapshot = detail.snapshot || {};
  document.querySelector('#detailTitle').textContent = `涓婃父璇︽儏锛?{detail.site.name}`;
  document.querySelector('#detailSubtitle').textContent = `${detail.site.base_url} 路 ${timeText(detail.site.last_sync_at)}`;
  document.querySelector('#detailContent').innerHTML = `
    <div class="detail-grid">
      <article class="metric"><span>浣欓</span><strong>${snapshot.balance ?? '涓嶅彲鐢?}</strong></article>
      <article class="metric"><span>鍏呭€兼瘮渚?/span><strong>${escapeHtml(rechargeText(snapshot))}</strong></article>
      <article class="metric"><span>鏀粯濂楅</span><strong>${Number(snapshot.payment_plan_count || 0)}</strong></article>
      <article class="metric"><span>鎬?Token</span><strong>${tokenText(snapshot.total_tokens)}</strong></article>
      <article class="metric"><span>浠婃棩 Token</span><strong>${tokenText(snapshot.today_tokens)}</strong></article>
      <article class="metric"><span>杩?7 澶?Token</span><strong>${tokenText(snapshot.week_tokens)}</strong></article>
      <article class="metric"><span>杩?30 澶?Token</span><strong>${tokenText(snapshot.month_tokens)}</strong></article>
      <article class="metric"><span>鎬绘垚鏈?/span><strong>${money.format(snapshot.total_cost || 0)}</strong></article>
      <article class="metric"><span>杩?7 澶╂垚鏈?/span><strong>${money.format(snapshot.week_cost || 0)}</strong></article>
      <article class="metric"><span>杩?30 澶╂垚鏈?/span><strong>${money.format(snapshot.month_cost || 0)}</strong></article>
      <article class="metric"><span>Key 鏁伴噺</span><strong>${snapshot.key_count || 0}</strong></article>
      <article class="metric"><span>娓犻亾鏁伴噺</span><strong>${snapshot.channel_count || 0}</strong></article>
      <article class="metric"><span>鍊嶇巼鑼冨洿</span><strong>${rateText(snapshot.min_rate)} - ${rateText(snapshot.max_rate)}</strong></article>
      <article class="metric"><span>妯″瀷骞垮満</span><strong>${Number(parsePricingSummary(snapshot).model_count || 0)}</strong></article>
    </div>
    ...
    <div class="detail-jump-actions">
      <button class="secondary" data-scroll-target="modelPricingSection">鏌ョ湅妯″瀷骞垮満鍊嶇巼</button>
    </div>
    ...
    <div class="rate-grid compact">
      ${(detail.rates || []).slice(0, 12).map((rate) => `
        <article class="rate-pill">
          ...
          <span>${rateText(rate.rate)}</span>
          <small>${escapeHtml(rate.scope || 'unknown')} 路 ${timeText(rate.captured_at)}</small>
        </article>
      `).join('') || '<p class="empty">鏆傛棤鍊嶇巼鏁版嵁銆?/p>'}
    </div>
    <h3>鏈€杩戝悓姝ヨ褰?/h3>
    <div class="list">
      ${(detail.logs || []).slice(0, 8).map((log) => `
        <div class="list-item">
          <strong>${escapeHtml(log.status)} 路 ${timeText(log.started_at)}</strong>
          <small>${escapeHtml(log.summary || log.error_message || '鏃犳憳瑕?)}</small>
        </div>
      `).join('') || '<p class="empty">鏆傛棤鍚屾璁板綍銆?/p>'}
    </div>
  `;
...
async function refresh() {
  ...
  document.querySelector('#lastUpdated').textContent = `鏈€鍚庡埛鏂帮細${new Date().toLocaleString('zh-CN')}`;
}
...
document.querySelector('#importFileInput').addEventListener('change', async (event) => {
  ...
  try {
    ...
    document.querySelector('#importPanel').hidden = false;
    document.querySelector('#importSummary').textContent = `瀵煎叆澶辫触锛?{err.message}`;
...
document.querySelector('#upstreamForm').addEventListener('submit', async (event) => {
  ...
  showMessage(id ? '涓婃父宸叉洿鏂般€? : '涓婃父宸蹭繚瀛樸€?, 'success');
  setForm();
  await refresh();
});

document.querySelector('#testConnectionBtn').addEventListener('click', async (event) => {
  await withButton(event.currentTarget, '娴嬭瘯涓?..', async () => {
    const payload = getFormPayload();
    delete payload.id;
    const result = await api('/api/upstreams/test', { method: 'POST', body: JSON.stringify(payload) });
    showMessage(`杩炴帴鎴愬姛锛氫綑棰?${result.snapshot.balance ?? '涓嶅彲鐢?}锛屽厖鍊?${rechargeText(result.snapshot)}锛屽€嶇巼 ${result.rates_count} 涓紝Key ${result.keys_count} 涓€俙, 'success');
  });
});
...
document.addEventListener('click', async (event) => {
  ...
  if (copyUrl) {
    ...
    event.target.textContent = '宸插鍒?;
    setTimeout(() => { event.target.textContent = '澶嶅埗'; }, 1200);
  }

  const syncId = event.target?.dataset?.syncId;
  if (syncId) {
    await withButton(event.target, '鍚屾涓?..', async () => {
  ...
  if (deleteId) {
    ...
    if (!confirm(`纭畾鍒犻櫎涓婃父銆?{site?.name || deleteId}銆嶅悧锛熻繖浼氬悓鏃跺垹闄ゅ畠鐨勫揩鐓у拰鏃ュ織銆俙)) return;
  ...
  if (createRechargeId) {
    await withButton(event.target, '鍒涘缓涓?..', async () => {
  ...
  if (refreshRechargeId) {
    await withButton(event.target, '鍒锋柊涓?..', async () => {
...
document.querySelector('#syncAllBtn').addEventListener('click', async (event) => {
  await withButton(event.currentTarget, '鍚屾涓?..', async () => {
...
bootstrapAuth().catch((err) => {
  ...
  document.querySelector('#lastUpdated').textContent = `鍔犺浇澶辫触锛?{err.message}`;
});
