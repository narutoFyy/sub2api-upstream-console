...
上游监控主页会按 24 小时、7 天、30 天展示消耗排行、消耗速度和预计可用时间。优先使用历史快照中的累计实际成本增量；累计成本重置或充值不会生成负消耗，缺少成本数据时才显示余额变化估算。历史采样不足一小时或少于两个有效点时，速度和预计可用时间显示为数据不足。

在上游监控首页可以直接同步全部或单个上游支持的模型，同步结果会更新各分组的 Key 模型候选。选择模型只保存配置，不会自动发起请求；点击对应 Key 的“立即检测”才会执行一次真实连通性测试。
