...
- 上游监控首页可直接同步全部或单个上游支持的模型，并为每个 Key 选择并保存检测模型；只有点击“立即检测”才会发起真实模型请求。
- 记录同步日志、最近成功时间、最近失败原因。
- 支持低余额、长期未同步、同步失败和倍率变化提醒。
- 支持 PushPlus 微信告警：可在设置页加密保存多个 Token，按目标启用/停用并单独测试；每条告警广播给所有启用目标，单个目标失败不阻塞其他目标。兼容旧的单 Token 配置和 `PUSHPLUS_TOKEN` 环境变量回退。可自定义事件开关、连续失败/恢复阈值、重复提醒、免打扰和恢复通知；默认把同一轮同一上游的多个 Key 异常合并成一条微信消息。
...
- 将上游列表的同步、模型同步、详情和永久删除统一收进操作菜单。
- 操作列固定在表格右侧，菜单提升到视口层级，横向滚动和末行场景下仍可直接操作。

### v1.8.6 - 2026-08-04

- 上游列表操作区增加永久删除入口，仍保留两次确认和本地级联清理保护。
- 更新前端资源版本参数，部署后浏览器不会继续使用旧版脚本缓存。
...
- 增加 Sub2API 刷新令牌的加密存储，访问令牌到期前会自动续期。
- 续期成功后持久化轮换后的访问令牌、刷新令牌和到期时间，无需重复处理 Turnstile 验证。

### v1.8.1 - 2026-07-22

- 自动发现同站点的真实 API 地址，适配入口域名跳转和静态服务导航页。
- 上游非 JSON/HTML 错误响应统一净化、脱敏并限制长度，不再把整页源码写入同步状态。
- 监控表格的同步错误改为单行摘要和悬停详情，修复长错误导致的页面横向溢出。
